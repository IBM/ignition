...
class TestLifecycleRequestQueueService(unittest.TestCase):
    ...
    def assert_request_failed_not_posted(self, request_as_dict):
        request = Request.from_str_message(json.dumps(request_as_dict), self.lifecycle_config.request_queue.failed_topic.name, 0, 0)
        self.mock_postal_service.post.assert_not_called_with(Envelope(self.lifecycle_config.request_queue.failed_topic.name, request.as_message()), key=request_as_dict["request_id"])
